...
I wanted to create this mode so that I can write more within Emacs without missing out on features, or extending existing features, that are included in other Word Processors
(e.g., MS Word, Libre Writer). I didn't want to have to pay for MS Word (though I may still have to if I ever want to do more complex document foramtting or am working with others)
and LibreOffice's look kinda throws me off (still very good software though!). So I thought, "why not just make that in my own config?", and here we are.
