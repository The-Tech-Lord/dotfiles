...
* Mode Settings
Even though some of these modes are obscure, I've created functions for them for the sake of completion.
...
(global-set-key (kbd "C-s") 'search-prfx)
#+END_SRC
...
*** Text Manipulation
By Text Manipulation, I don't refer to manipulating text in general but rather the text itself (e.g., changing the case of a letter from uppercase to lowercase).
...
(global-set-key (kbd "C-x b l") 'bookmark-lmenu-list)
#+END_SRC
