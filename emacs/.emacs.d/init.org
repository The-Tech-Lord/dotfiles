...
***** Functions
When called, it will allow one to adjust windows using the idea of Macros and Cents. C-{ and C-} will be used for Cent adjustments while C-[ and C-] will be
used for Macro adjustments.

An implementation idea is that I could create a window adjusment minor mode that will allow more options for creating key bindings.
#+BEGIN_SRC emacs-lisp
(defun window-adjust()
  )
#+END_SRC
...
***** Functions
Shows incoming and outgoing logs before one will want to commit.
If given a prefix, it will emphasize a type of logging command (e.g., vc-print-log). A hook may be used depending on how this is implemented.
#+BEGIN_SRC emacs-lisp
(defun pre-logs()
  )
#+END_SRC
...
Shows outgoing logs when comitting.
#+BEGIN_SRC emacs-lisp
(defun committing-pre-logs()
  )
#+END_SRC
