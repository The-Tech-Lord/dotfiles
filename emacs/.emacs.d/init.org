...
to traverse through (Eleanor Lamb saves her mother from drowning and all the little sisters are alive, the ending I got). For clarity, I got the idea from a quote from the
...
I wanted to create this mode so that I can write more personal documents within Emacs without missing out on features that are included in other Word Processors (e.g., MS Word,
Libre Writer). I didn't want to have to pay for MS Word (though I may still have to if I'm ever working with others) and LibreOffice's look kinda throws me off (still very good
software though!). So I thought, "why not just make that in my own config?", and here we are.
