...
* About
I actually used to use Vim, though, after a while I wanted to try out something different and decided to try GNU Emacs again (this being the third time). A reason why I went with
GNU Emacs was that it was lightweight, i.e., it wasn't a taking 300-500MB of RAM while idling. That may be a picky reason, though goes back to when I was using a DELL Inspiron 17
5000 2-in-1 (Intel Core i5, Intel UHD Graphics, 8GB RAM, one cooling fan). Even when just using VS Code, my laptops fans would start running.
...
* Bindings
The Bindings section is where you'll start to see my "wild" side. People have ventured here, only few returning with stories of a wacky, wild
adventure. The horrors that one would see could not be described with mere words. Others have sought out to witness for themselves the tortures the
other adventurers have faced. Soon, it'll be known that it was their greatest mistake.
...
* Plasmids
I love the Bioshock series and felt like naming mode definitions after the Bioshock series.
...
having to use convoluted keybindings that break your pinky. Of course for simple manipulations, such as correcting "theer" to "there", would just need a simple global or correcting
it manually.
#+BEGIN_SRC emacs-lisp
...
This is a relatively simple mode as it'll make adjusting and managing windows easier.
#+BEGIN_SRC emacs-lisp
