...
* About
I actually used to use Vim as my main editor, though, after a while I wanted to try out something different and decided to try GNU Emacs again (this being the third time). A reason
why I went with GNU Emacs was that it was lightweight, i.e., it wasn't a taking 300-500MB of RAM while idling. That may be a picky reason, though goes back to when I was using a
DELL Inspiron 17 5000 2-in-1 (Intel Core i5, Intel UHD Graphics, 8GB RAM, one cooling fan). Even when just using VS Code, my laptops fans would start running.

As you will see in this file, I'm an organization junkie.
...
****** Chunks
"Chunks" refers to when commands effect more than one character.
******* Words
#+BEGIN_SRC emacs-lisp
...
**** Manipulation
Commands here manipulate the style of the text (e.g., Cases) and the position of the text (e.g., Transposing, Splitting). Setting up this header was one of the harder one's to try
...
**** Indenting
Went with M-i as the prefix command here beacuse 1) indenting wouldn't be something I'd use very much, hence the Meta stroke, and 2) felt that C-i would probably cause problems if
I did ever use it as the indentation command prefix; it's also just slightly easier to type.
#+BEGIN_SRC emacs-lisp
...
** Eve Syringes
*** Delta
This mode I named after the main protagonist of Bioshock 2, the Big Daddy, Delta.
...
The purpose of this mode is to make text manipulation easier and also be able to provide more varied keybindings. It'll allow movement of many sentences and paragraphs without
having to use convoluted keybindings that break your pinky.  Of course for simple manipulations, such as correcting 'theer' to 'there', would just need a simple global or
...
*** Eleanor
This mode is named after the main objective of Delta, getting to his Little Sister, Eleanor Lamb. I went with this because of the isolated environment she lived in for her entire
life and wanted to associate it with something that is also brings a sense of freedom, Windows.

This is mode creates an easier experience for managing multiple windows once you enter the mode as you have access to simple keybindings.
